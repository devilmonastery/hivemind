...
func (r *quoteRepository) GetByID(ctx context.Context, id string, userDiscordID string) (*entities.Quote, error) {
	query := `
		SELECT q.id, q.body, q.author_id, q.author_discord_id, u.name, q.guild_id, dg.guild_name,
		       ...
		       gm_author.guild_nick, gm_source.guild_nick
		FROM quotes q
		LEFT JOIN discord_guilds dg ON q.guild_id = dg.guild_id
		LEFT JOIN users u ON q.author_id = u.id
		LEFT JOIN guild_members gm_author ON q.guild_id = gm_author.guild_id AND q.author_discord_id = gm_author.discord_id
		LEFT JOIN guild_members gm_source ON q.guild_id = gm_source.guild_id AND q.source_msg_author_discord_id = gm_source.discord_id`

	// Add ACL check if userDiscordID provided (non-admin)
	if userDiscordID != "" {
		query += `
		INNER JOIN guild_members gm_acl ON q.guild_id = gm_acl.guild_id AND gm_acl.discord_id = $2`
	}
	...
	var authorGuildNick, sourceAuthorGuildNick sql.NullString
	...
	if userDiscordID != "" {
		err = r.db.QueryRowContext(ctx, query, id, userDiscordID).Scan(
			...
			&authorGuildNick, &sourceAuthorGuildNick,
		)
	} else {
...
func (r *quoteRepository) List(ctx context.Context, guildID string, limit, offset int, orderBy string, ascending bool, userDiscordID string) ([]*entities.Quote, int, error) {
	...
	if userDiscordID != "" {
		...
		INNER JOIN guild_members gm_acl ON q.guild_id = gm_acl.guild_id AND gm_acl.discord_id = $%d`, argCount)
